...
export const users = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`btrim(${table.email}) <> '' AND position('@' in ${table.email}) > 1`,
    ),
    ...
    check(
      "users_study_languages_check",
      sql`cardinality(${table.studyLanguages}) > 0 AND ${table.studyLanguages} <@ ARRAY['en','ta','ml','hi']::text[]`,
    ),
  ],
);
...
export const userProgress = pgTable(
  ...
  (table) => [
    uniqueIndex("user_progress_user_item_uidx").on(
      table.userId,
      table.itemType,
      table.itemId,
    ),
    ...
    check(
      "user_progress_accuracy_check",
      sql`${table.accuracy} BETWEEN 0 AND 1`,
    ),
...
export const userBookmarks = pgTable(
  ...
  (table) => [
    uniqueIndex("user_bookmarks_user_item_collection_uidx").on(
      table.userId,
      table.itemType,
      table.itemId,
      table.collectionName,
    ),
    index("user_bookmarks_user_collection_idx").on(
      table.userId,
      table.collectionName,
    ),
    index("user_bookmarks_item_idx").on(table.itemType, table.itemId),
    check(
      "user_bookmarks_collection_not_blank_check",
      sql`btrim(${table.collectionName}) <> ''`,
    ),
  ],
);
