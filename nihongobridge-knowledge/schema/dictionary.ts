...
export const dictionaryEntries = pgTable(
  "dictionary_entries",
  {
    ...
    pitchAccent: jsonb("pitch_accent").$type<PitchAccent | null>(),
  ...
  (table) => [
    ...
    index("dictionary_entries_jlpt_frequency_idx").on(
      table.jlptLevel,
      table.frequencyRank,
    ),
    index("dictionary_entries_word_trgm_idx").using(
      "gin",
      sql`${table.word} gin_trgm_ops`,
    ),
    index("dictionary_entries_kana_trgm_idx").using(
      "gin",
      sql`${table.kana} gin_trgm_ops`,
    ),
    index("dictionary_entries_romaji_trgm_idx").using(
      "gin",
      sql`${table.romaji} gin_trgm_ops`,
    ),
    ...
    index("dictionary_entries_fts_idx").using(
      "gin",
      sql`(
    ...
    check(
      "dictionary_entries_frequency_rank_check",
      sql`${table.frequencyRank} IS NULL OR ${table.frequencyRank} > 0`,
    ),
    check(
      "dictionary_entries_word_not_blank_check",
      sql`btrim(${table.word}) <> ''`,
    ),
    check(
      "dictionary_entries_source_not_blank_check",
      sql`btrim(${table.source}) <> ''`,
    ),
    check(
      "dictionary_entries_furigana_array_check",
      sql`jsonb_typeof(${table.furigana}) = 'array'`,
    ),
    check(
      "dictionary_entries_meanings_array_check",
      sql`jsonb_typeof(${table.meanings}) = 'array'`,
    ),
    check(
      "dictionary_entries_pitch_accent_object_check",
      sql`${table.pitchAccent} IS NULL OR jsonb_typeof(${table.pitchAccent}) = 'object'`,
    ),
  ],
);
