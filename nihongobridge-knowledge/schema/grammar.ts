...
export const grammarPatterns = pgTable(
  "grammar_patterns",
  {
    ...
    formationDiagram: jsonb("formation_diagram").$type<FormationDiagram | null>(),
  ...
  (table) => [
    ...
    index("grammar_patterns_pattern_trgm_idx").using(
      "gin",
      sql`${table.pattern} gin_trgm_ops`,
    ),
    index("grammar_patterns_plain_trgm_idx").using(
      "gin",
      sql`${table.patternPlain} gin_trgm_ops`,
    ),
    ...
    index("grammar_patterns_fts_idx").using(
      "gin",
      sql`(
    ...
    check(
      "grammar_patterns_meaning_array_check",
      sql`jsonb_typeof(${table.meaning}) = 'array'`,
    ),
    check(
      "grammar_patterns_examples_array_check",
      sql`jsonb_typeof(${table.examples}) = 'array'`,
    ),
    check(
      "grammar_patterns_formation_diagram_object_check",
      sql`${table.formationDiagram} IS NULL OR jsonb_typeof(${table.formationDiagram}) = 'object'`,
    ),
  ],
);
