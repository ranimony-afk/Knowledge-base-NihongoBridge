...
export const sentences = pgTable(
  ...
  (table) => [
    ...
    index("sentences_japanese_trgm_idx").using(
      "gin",
      sql`${table.japanese} gin_trgm_ops`,
    ),
    ...
    check(
      "sentences_translations_array_check",
      sql`jsonb_typeof(${table.translations}) = 'array'`,
    ),
  ],
);
