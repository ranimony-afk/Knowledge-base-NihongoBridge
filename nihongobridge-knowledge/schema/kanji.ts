...
export const kanjiEntries = pgTable(
  ...
  (table) => [
    ...
    index("kanji_entries_jlpt_frequency_idx").on(
      table.jlptLevel,
      table.frequencyRank,
    ),
    ...
    index("kanji_entries_meaning_fts_idx").using(
      "gin",
      sql`to_tsvector('english', coalesce(${table.meanings}::text, ''))`,
    ),
    check(
      "kanji_entries_grade_check",
      sql`${table.grade} IS NULL OR ${table.grade} BETWEEN 1 AND 9`,
    ),
    check(
      "kanji_entries_frequency_rank_check",
      sql`${table.frequencyRank} IS NULL OR ${table.frequencyRank} > 0`,
    ),
    check(
      "kanji_entries_stroke_count_check",
      sql`${table.strokeCount} IS NULL OR ${table.strokeCount} > 0`,
    ),
    check(
      "kanji_entries_character_check",
      sql`char_length(btrim(${table.character})) = 1`,
    ),
    check(
      "kanji_entries_meanings_array_check",
      sql`jsonb_typeof(${table.meanings}) = 'array'`,
    ),
    check(
      "kanji_entries_mnemonics_array_check",
      sql`jsonb_typeof(${table.mnemonics}) = 'array'`,
    ),
    check(
      "kanji_entries_source_not_blank_check",
