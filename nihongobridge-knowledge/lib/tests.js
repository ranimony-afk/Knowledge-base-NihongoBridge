...
import { boolean, check, doublePrecision, index, integer, jsonb, pgTable, text, timestamp, uuid, } from "drizzle-orm/pg-core";
import { jlptLevelEnum, jlptTestLevelEnum, questionSectionTypeEnum, testTypeEnum, } from "./enums.js";
...
export const practiceTests = pgTable("practice_tests", {
    ...
    index("practice_tests_level_type_published_idx").on(table.level, table.testType, table.isPublished),
    ...
    check("practice_tests_sections_array_check", sql `jsonb_typeof(${table.sections}) = 'array'`),
    check("practice_tests_total_time_check", sql `${table.totalTimeMinutes} > 0`),
    check("practice_tests_difficulty_score_check", sql `${table.difficultyScore} BETWEEN 0 AND 5`),
]);
export const questions = pgTable("questions", {
    ...
    index("questions_level_section_difficulty_idx").on(table.jlptLevel, table.sectionType, table.difficulty),
    ...
    index("questions_fts_idx").using("gin", sql `(
    ...
    check("questions_text_present_check", sql `coalesce(btrim(${table.questionJp}), '') <> '' OR coalesce(btrim(${table.questionEn}), '') <> ''`),
    check("questions_options_array_check", sql `jsonb_typeof(${table.options}) = 'array'`),
    check("questions_options_count_check", sql `jsonb_array_length(${table.options}) BETWEEN 2 AND 8`),
    check("questions_correct_answer_exists_check", sql `${table.options} @> jsonb_build_array(jsonb_build_object('id', ${table.correctAnswer}))`),
    check("questions_stimulus_object_check", sql `${table.stimulus} IS NULL OR jsonb_typeof(${table.stimulus}) = 'object'`),
    check("questions_difficulty_check", sql `${table.difficulty} BETWEEN 1 AND 5`),
    check("questions_time_limit_check", sql `${table.timeLimitSeconds} IS NULL OR ${table.timeLimitSeconds} > 0`),
    check("questions_source_check", sql `${table.source} IN ('original', 'generated')`),
]);
export const testSessions = pgTable("test_sessions", {
    ...
    scoreBySection: jsonb("score_by_section").$type(),
    ...
    check("test_sessions_completed_after_start_check", sql `${table.completedAt} IS NULL OR ${table.completedAt} >= ${table.startedAt}`),
    check("test_sessions_answers_array_check", sql `jsonb_typeof(${table.answers}) = 'array'`),
    check("test_sessions_score_total_check", sql `${table.scoreTotal} IS NULL OR ${table.scoreTotal} BETWEEN 0 AND 180`),
    check("test_sessions_score_by_section_object_check", sql `${table.scoreBySection} IS NULL OR jsonb_typeof(${table.scoreBySection}) = 'object'`),
]);
