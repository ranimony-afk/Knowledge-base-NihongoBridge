...
import { boolean, check, doublePrecision, foreignKey, index, integer, pgTable, text, timestamp, unique, uniqueIndex, uuid, } from "drizzle-orm/pg-core";
import { contentItemTypeEnum, jlptLevelEnum, srsConfidenceEnum, } from "./enums.js";
...
export const srsCards = pgTable("srs_cards", {
    ...
    uniqueIndex("srs_cards_user_item_uidx").on(table.userId, table.itemType, table.itemId),
    index("srs_cards_due_idx").on(table.userId, table.nextReviewAt, table.easeFactor),
    ...
    check("srs_cards_ease_factor_check", sql `${table.easeFactor} BETWEEN 1.3 AND 2.5`),
    ...
    check("srs_cards_review_counts_check", sql `${table.correctCount} + ${table.mistakeCount} <= ${table.totalReviews}`),
]);
...
export const srsReviewLogs = pgTable("srs_review_logs", {
    ...
    check("srs_review_logs_interval_check", sql `${table.previousIntervalDays} >= 1 AND ${table.nextIntervalDays} >= 1`),
    check("srs_review_logs_ease_check", sql `${table.previousEaseFactor} BETWEEN 1.3 AND 2.5 AND ${table.nextEaseFactor} BETWEEN 1.3 AND 2.5`),
]);
